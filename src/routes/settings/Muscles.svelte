<script>
  ...
  import { exerciseMuscles, catName as _catName, catDisplay as _catDisplay } from '../../stores/settings.js';
  ...
  function openEmojiPicker(e) {
    ...
    picker.addEventListener('emoji-click', ev => {
      newMuscleLabel = ev.detail.unicode;
      _destroyEmojiPicker();
    });
  ...
  function addMuscle() {
    ...
    const label = newMuscleLabel.trim();
    exerciseMuscles.set([...cats, label ? { name, label } : name]);
    newMuscleName = '';
    newMuscleLabel = '';
  }
  function removeMuscle(cat) {
    const n = _catName(cat);
    exerciseMuscles.set((get(exerciseMuscles) || []).filter(c => _catName(c) !== n));
  }
</script>

<div class="section-body">
  <div class="card settings-card">
    <p class="setting-desc" style="padding:14px 16px 0">{$_('settings.muscles.help')}</p>
    <div class="cat-chips-wrap">
      {#each ($exerciseMuscles || []) as cat}
        <div class="chip">
          {_catDisplay(cat)}
          <button class="chip-x" on:click={() => removeMuscle(cat)} aria-label={$_('common.delete')}>
            <span class="material-symbols-rounded" style="font-size:14px">close</span>
          </button>
        </div>
      {/each}
      {#if ($exerciseMuscles || []).length === 0}
        <span class="text-3 text-sm">{$_('settings.muscles.empty')}</span>
      {/if}
    </div>
    ...
    <div class="cat-add-row">
      <div style="display:flex;flex-direction:column;gap:3px;flex-shrink:0;position:relative">
        ...
        <button class="input emoji-btn" title={$_('settings.muscles.pick_label')}
          on:click={openEmojiPicker}>
          {newMuscleLabel || '🏷️'}
        </button>
      </div>
      <div style="display:flex;flex-direction:column;gap:3px;flex:1">
        <span style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--text-3)">{$_('settings.muscles.name_label')}</span>
        <input class="input" style="height:40px" placeholder={$_('settings.muscles.name_placeholder')}
          bind:value={newMuscleName} on:keydown={e => e.key==='Enter' && addMuscle()} />
      </div>
      <button class="btn btn-secondary" style="height:40px;padding:0 16px;align-self:flex-end" on:click={addMuscle}>{$_('exercises.add')}</button>
    </div>
  </div>
</div>
